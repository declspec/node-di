...
function createInjector(modulesToLoad, strictDi) {
    ...
    var providerInjector = providerCache.$injector = createInternalInjector(providerCache, function(name, caller) {
        ...
        if ("string" === typeof(caller))
            path.push(caller);
        throw injectorErr("unpr", "Unknown provider: {0}", path.join("\n\t"));
    });
        ...
        function getService(serviceName, caller) {
            if (cache.hasOwnProperty(serviceName)) {
                if (cache[serviceName] === INSTANTIATING)
                    throw injectorErr("cdep", "Circular dependency found: {0}", serviceName + ' <- ' + path.join(' <- '));
